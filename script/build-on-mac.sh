...
cd $PROJECT_DIR
## replace string fom mac, if not on mac ,shoul not to use -is
sed -is 's/RunMode = dev/RunMode = prod/' conf/app.conf && rm conf/app.confs
cat conf/app.conf | grep RunMode
...
cp conf/diss-backend.prod.env $BUILD_DIR/conf/diss-backend.env
sed -is 's/RunMode = prod/RunMode = dev/' conf/app.conf && rm conf/app.confs
...
echo "=========== 8. push images to registry ==========="
docker push registry.cn-qingdao.aliyuncs.com/diss/diss-backend:latest



echo "=========== 9. build success ==========="
