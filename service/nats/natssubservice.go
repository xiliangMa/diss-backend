			...
			if cc[models.WarningInfoId] == nil {
				...
				if r := resp.Get(); r != nil {
					if obj[models.StatusKey] == "" {
						...
						r.Status = "失败"
						r.Code = 1400
						r.Msg = obj[models.StatusKey].(string)
					}
				...
				if obj[models.StatusKey] == "" {
					resp.Status = "成功"
					resp.Msg = models.Status
				} else {
					resp.Status = "失败"
					resp.Code = 1400
					resp.Msg = obj[models.StatusKey].(string)
				}
...
func (this *NatsSubService) DeleteTask() error {
	...
	if subject != "" {
		...
		return errors.New(string(rune(utils.ResourceNotFoundErr)))
	}
	if err != nil {
