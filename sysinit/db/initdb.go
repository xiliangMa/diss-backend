...
func (this *DefaultDB) registerModel() {
	...
	orm.RegisterModel(new(models.LogConfig), new(models.TimeEdgePoint), new(models.WarningWhiteList), new(models.RespCenter), new(models.RuleDefine), new(models.Version))
	// license
...
func (this *DefaultDB) InitSystemData() {
	...
	_, err = o.Raw(dbscript.DefaultContainerVirusScanSql).Exec()
	if err != nil {
	...
	_, err = o.Raw(dbscript.DefaultContainerVirusScanJobSql).Exec()
	if err != nil {
		logs.Error("Init DefaultContainerVirusScanJobSql Job fail, err: %s", err)
	}
	...
	_, err = o.Raw(dbscript.DefaultImageVirusScanJobSql).Exec()
	if err != nil {
		logs.Error("Init DefaultImageVirusScanJobSql Job fail, err: %s", err)
	}
